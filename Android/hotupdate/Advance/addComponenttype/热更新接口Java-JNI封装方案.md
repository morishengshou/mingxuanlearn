...
|---|---|---|
| `std::vector<std::string> gethotupdatefiles(int componentType)` | `nativeGetHotUpdateFiles(int componentType) : String[]` | `ArrayList<String> getHotUpdateFiles(int componentType)` |
| `std::vector<std::string> gethotupdateprops(int componentType)` | `nativeGetHotUpdateProps(int componentType) : String[]` | `ArrayList<String> getHotUpdateProps(int componentType)` |
| `char* GetHotUpdateFile(const char* pathSuffix, int type, int componentType, char* buf, int bufLen)` | `nativeGetHotUpdateFile(String pathSuffix, int type, int componentType) : String` | `File getHotUpdateFile(String pathSuffix, int type, int componentType)` |

### 关于 `GetHotUpdateFile` 的参数

- **`pathSuffix`、`type`、`componentType` 与 C++ 完全一致**，直接透传（`componentType` 的过滤语义由 C++ 侧定义，JNI 层不解释）。
...
int componentType = 0;   // 业务定义的组件类型，透传给 native 做过滤

ArrayList<String> files = HotUpdateManager.getHotUpdateFiles(componentType);   // 与 C++ 结果逐字节一致
ArrayList<String> props = HotUpdateManager.getHotUpdateProps(componentType);

File f = HotUpdateManager.getHotUpdateFile("lib/libfoo.so", HotUpdateManager.TYPE_FILE, componentType);
if (f != null && f.canRead()) {
