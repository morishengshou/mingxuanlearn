...
public final class HotUpdateManager {
    ...
    /**
     * 读取指定组件类型的所有热更新文件名。
     * @param componentType 组件类型过滤条件，透传给 native，过滤语义由 C++ 侧定义
     * @return 文件不存在 / 无记录 / 库不可用时返回空列表，永不返回 null
     */
    public static ArrayList<String> getHotUpdateFiles(int componentType) {
        if (!NATIVE_AVAILABLE) {
            return new ArrayList<>();
        }
        String[] arr = nativeGetHotUpdateFiles(componentType);
     ...
     * @return 文件不存在 / 无记录 / 库不可用时返回空列表，永不返回 null
     */
    public static ArrayList<String> getHotUpdateProps(int componentType) {
        if (!NATIVE_AVAILABLE) {
            return new ArrayList<>();
        }
        String[] arr = nativeGetHotUpdateProps(componentType);
        return arr == null ? new ArrayList<>() : new ArrayList<>(Arrays.asList(arr));
    }
     ...
     * @param componentType 组件类型过滤条件，透传给 native
     */
    public static File getHotUpdateFile(String pathSuffix, int type, int componentType) {
        if (!NATIVE_AVAILABLE || pathSuffix == null || pathSuffix.isEmpty()) {
            return null;
        }
        String path = nativeGetHotUpdateFile(pathSuffix, type, componentType);
        return path == null ? null : new File(path);
    }

    // === native 方法（经 JNI_OnLoad 中 RegisterNatives 绑定）===
    private static native String[] nativeGetHotUpdateFiles(int componentType);
    private static native String[] nativeGetHotUpdateProps(int componentType);
    private static native String nativeGetHotUpdateFile(String pathSuffix, int type, int componentType);
}
```
...
**结论**：本方案（内置进 client 库）在内存上**不比独立 `.so` 多，反而更省**；相较纯 Java 直读，
会多一份 native 库映射，但增量很小、且大部分可跨进程共享。
...
| 段 | 归属 | 说明 |
|---|---|---|
| 代码 / 只读段（`.text` / `.rodata`） | **跨进程共享** | 系统内其他进程（如 system_server）已加载时，App 再加载几乎零边际物理内存；即便独占，也是干净页、可回收 |
| 可写段（`.data` / `.bss` / GOT 重定位页） | **每进程私有（dirty）** | 真正占用的部分；对这种小库通常仅**几 KB～几十 KB** |

所以「加载一个库」落到 App 头上的私有内存，主要就是那几十 KB 级别的私有页 + 少量链接器簿记（soinfo）。

#### (b) 三种路线对比

| 路线 | App 进程内新增 native 映射 | 相对内存 |
|---|---|---|
| 纯 Java 直读 | 无 | 最低（但逻辑在 Java 侧另有一份） |
| **内置进 client 库（本方案）** | **1 个** `.so`（client，含 JNI） | 居中，比独立库省 |
| 独立 `libhotupdate_jni.so` | **2 个** `.so`（jni + 依赖的 client） | 最高（多一套每进程固定开销） |

**内置比独立省的原因**：独立方案要加载 jni + client **两个**库，多一套链接器开销、GOT/重定位私有页、
最小映射粒度对齐；内置只加载 **一个** client 库，JNI 那几个函数的 `.text` 增量可忽略。**少一个库 = 少一套每进程固定开销。**

#### (c) 为什么整体增量很小

1. `libhotupdate_common` 是**静态链接**进 client，不产生额外 `.so`；
2. 传递依赖 `liblog` / `libc` / `libc++`——**每个 App 进程本来就已加载**，不新增；
3. JNI 不持有 global ref、局部引用即时释放，ART 侧只多几条 `RegisterNatives` 记录，可忽略；
4. 库体积小，代码页还能跨进程共享。

#### (d) 可选优化：懒加载

目前 `HotUpdateManager` 在静态块里 `System.loadLibrary`（类加载即映射）。若这几个接口调用很稀少，
可改为**首次调用时才加载**，平时不占内存，代价是首调多一点延迟。仅在极端内存敏感且 API 罕用时才需如此，
常规业务静态加载即可。

#### (e) 实测方法

```sh
adb shell dumpsys meminfo <包名>            # 看 .so 相关的 Private Dirty / PSS
adb shell showmap <pid> | grep hotupdate    # 看该 .so 各段私有/共享页
```

关注 `libhotupdate_client.so` 那几行的 **Private Dirty**——即它给该进程新增的实打实内存，通常为几十 KB 量级。
